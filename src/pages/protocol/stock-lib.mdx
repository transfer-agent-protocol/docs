...
The `Stock` library contains functions to manage stock transactions, including issuance, transfer, cancellation, reissuance, repurchase, and retraction. The library also includes error definitions and internal utility functions to support these operations.
...
When to use: used when a stakeholder needs to accept stock that has been issued to them.
